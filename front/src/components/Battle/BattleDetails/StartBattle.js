...
const StartBattle = ({
  ...
  const hitEffected = (e) => {
    ...
    setNotifications(
      `${
        firstPokemon.nickname || firstPokemon.name
      } use ${e.target.innerText.toUpperCase()}!`
    );
    if (rnd === 0) {
  ...
  const rivalHitEffected = () => {
    ...
    if (rnd !== 0) {
      if (rivalMoves.length === 1) {
        setTimeout(() => {
          setNotifications(
            `${rival.name} use ${rivalMoves[0].name.toUpperCase()}!`
          );
        }, 1000);
      }
      setTimeout(() => {
        setNotifications(
          `${rival.name} use ${rivalMoves[rndMove].name.toUpperCase()}!`
        );
      }, 1000);
    }
    ...
    setTimeout(
      () =>
        setNotifications(
          `What will ${firstPokemon.nickname || firstPokemon.name} do?`
        ),
      3000
    );
    ...
    if (current <= 0) {
      ...
      setTimeout(() => {
        ...
        setNotifications(
          `${firstPokemon.nickname || firstPokemon.name} fainted`
        );
        setPausaForMe(true);
      }, 2500);
  ...
  const catchChance = (chanceRate) => {
    ...
    if (rndChances % chanceRate === 0) {
      setPokemonInBall(true);
      setTimeout(() => {
        setNotifications(`You Catched a ${rival.name}`);
      }, 2700);
      team.push(rival);
      setItemToLocalStorage(`myteam${user.user._id}`, team);
      setTimeout(() => {
  ...
  const useMedicine = () => {
    ...
    if (
      currentMyHp < firstPokemon.stats[0].base_stat &&
      numberOfMedicines > 0
    ) {
      setNotifications(
        `${firstPokemon.nickname || firstPokemon.name} use medicine`
      );
      ...
      setNotifications(
        `${firstPokemon.nickname || firstPokemon.name} have full HP`
      );
      setTimeout(
        () =>
          setNotifications(
  ...
  return (
    <Box style={{ textAlign: "center" }} sx={style}>
      ...
      {rivalPokemons + 1 <= battleArr.length && (
        <h3 className="battleHeader">
          {" "}
          Round {rivalPokemons + 1} from {battleArr.length}
        </h3>
      )}
      ...
      <div style={{ paddingTop: "1rem" }}>
        <div>
          <CardGroup>
            ...
            />
            <div style={{ paddingLeft: "20rem", display: "flex" }}></div>
            <StartBattleContent
          ...
          <div className="wrapper battleOperations">
            ...
            <div
              style={{ float: "right", maxWidth: "22rem", height: "4.5rem" }}
              className="second"
            >
              <Fragment>
                {myTurn === true && showAttacks === false && (
                  <>
                    ...
                    {team.length !== 6 && (
                      ...
                      >
                        <MdCatchingPokemon style={{ fontSize: "1.4rem" }} /> X{" "}
                        {numberOfPokeballs}
                      </Button>
                    )}
                    {team.length === 6 && (
                    ...
                    {numberOfMedicines !== 0 && (
                      <Button
                        ...
                        onClick={useMedicine}
                      >
                        <AiOutlineMedicineBox style={{ fontSize: "1.4rem" }} />{" "}
                        X {numberOfMedicines}
                      </Button>
                    )}
                    {numberOfMedicines === 0 && (
              ...
              {myMoves.slice(0, 4).map((move, idx) => (
                <Fragment key={idx}>
                  {myTurn === true && (
                    <Button
                      style={{ width: "11rem", color: "black" }}
