...
// const StartBattle = ({ style, battleArr, setBattleArr, themeSlice, rival, setOtherPokemon, setSwitcher, levelUpHandler }) => {
...
//         setCurrentRivalHp(rival.stats[0].base_stat)
//         setMyTurn(false)
//         setTimeout(() => { setNotifications(`A wild ${rival.name} appeared!`) }, 3300)
//         setTimeout(() => {
...
//             </IconButton>
//             {rivalPokemons + 1 <= battleArr.length && <h3 className='battleHeader'>{rivalPokemons + 1 <= battleArr.length ? `Round ${rivalPokemons + 1} from ${battleArr.length}` : 'You Win! \n\n'}</h3>}
