  ...
  const getPokemon = useCallback(
    async (e) => {
      ...
      pokemonArr.map(async (pokemon) => {
        ...
        if (
          pokemonName.startsWith(e.target.value) &&
          pokemonName.includes(e.target.value)
        ) {
          setDisplay(true);
    ...
    <div>
      <div>
        ...
        {display && (
          <div
            className={
              themeSlice === false ? "auto-container-dark" : "auto-container"
            }
          >
            <ul
              className={
                themeSlice === false
                  ? "autocomplete-items-dark"
                  : "autocomplete-items"
              }
              ref={itemRef}
            >
              {inputRef.current.value !== "" && (
                <li
                  className={
                    themeSlice === false
                      ? "autocomplete-res-dark"
                      : "autocomplete-res"
                  }
                >
                  <p className="res-counter">
                    Found {pokemonData.length} Results for "
                    {inputRef.current.value}"
                  </p>
                </li>
              )}
