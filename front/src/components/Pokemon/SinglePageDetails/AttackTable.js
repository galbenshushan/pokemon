    ...
    const getDetails = async () => {
        ...
        moves.map(async (move, idx) => {
            ...
            if (move.version_group_details[0].level_learned_at === 0) {
                setPokemonAttacksByTm(current => [...current, { name, power, accuracy, type, textToMap, pp }])
            }
            if (move.version_group_details[0].level_learned_at > 0) {
                setPokemonAttacksByLv(current => [...current, { name, power, accuracy, type, atLv, textToMap, pp }])
            }
        })
    }
