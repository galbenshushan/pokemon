...
const WhosThatPokemon = () => {
    ...
    const answerCheck = () => {
        ...
        setTimeout(() => setAnswers(answers.sort((a, b) => 0.5 - Math.random())), 1000);
        setTimeout(() => setShowPokemonFlag(false), 1000);
        setTimeout(() => setQuestionsFlag(state => state + 1), 1000);
        dispatch(answersActions.addAnswers({ user_answer: userAnswer, correct_answer: pokemon.name }))
        setUserAnswer(null)
    }
    ...
    return (<div className={showPokemonFlag === false ? 'animate__animated animate__backInLeft' : ''}>
        <div className='layout'>
            {questionsFlag < 5 && <Card style={{ backgroundColor: themeSlice === false ? 'rgb(12, 12, 12)' : 'rgb(227, 236, 243)' }} className=' who'>
                <CardHeader
                    title={<img className='imgWho' src="https://fontmeme.com/permalink/220124/53c2fe7c21b06c4c732f3a738e29bace.png" alt="pokemon-font" />} />
                <section >
                    <div tabIndex='0' onKeyDown={keysEvents} className='containe'>
                        <h5 style={{ width: '20rem', display: showPokemonFlag === false ? 'none' : 'inline' }}>
                            It's {pokemon.name}!
                        </h5>
                        <img alt="pokemon" src={pokemon.image} className='whosThatPokemon' style={{ width: '20rem', transition: '0.5s', filter: showPokemonFlag === false ? 'brightness(0)' : 'brightness(1)' }} />
                        ...
                        <RadioGroup
                            ...
                            {answers.map((answer, idx) => <div key={idx}>
                                <FormControlLabel onClick={answerHandler} value={answer.name} control={<Radio style={{ color: dynamicText }} />} label={`${idx + 1}. ${answer.name} `} />
                                <br />
                            </div>)}
                        </RadioGroup>
                        {userAnswer !== null && <>
                            <>{user.user.first_name === '' ? '' : user.user.first_name}, please click when you sure. <br /></>
                        </>}
                        {userAnswer && <button style={{ zIndex: '999' }} onClick={answerCheck} className='btn btn-danger'>Check</button>}
                    </div>
                </section>
            </Card>}
            {questionsFlag === 5 && <Card style={{ backgroundColor: themeSlice === false ? 'rgb(12, 12, 12)' : 'rgb(227, 236, 243)' }} className=' whoEnd'>
                <CardHeader
                    title={<h1 style={dynamicText}>Dear {user.user.first_name}</h1>} />
                <h5 style={dynamicText}>You answered {answersSelector.correct} questions correctly out of {answersSelector.total}</h5>
                {answersSelector.correct === 5 && team.length < 6 && <>
                    <p style={dynamicText}>Now you can pick your reward!</p>
                    {showRewardFlag === false && <Button onClick={() => setShowRewardFlag(true)} variant="contained" color='error' style={dynamicText}>Click to see what you earned</Button>}
                    {showRewardFlag === true && <>
                        <img alt="pokemon" src={pokemon.image} className='whosThatPokemon' style={{ width: '20rem', transition: '0.5s' }} />
                        <p style={dynamicText}>Your reward is {pokemon.name}!</p>
                        <Button onClick={addToTeamHandler} variant="contained" color='error' style={dynamicText}>Click to Add {pokemon.name} to your team</Button>
                    </>}
                </>
                }
                {answersSelector.correct === 5 && team.length === 6 && <>
                    <p style={dynamicText}>We are sorry, your team is full so you can't get any rewards right now..</p>
                </>
                }
            </Card>}
        </div>
